...
---@param root string
---@return string[]
function libopk.getWorld(root)
	local world = {}
	for line in io.lines(root .. "/etc/opk/world") do
...
function libopk.getInstalled(root)
	local installed = {}
	for line in io.lines(root .. "/etc/opk/installed") do
		local parts = string.split(line, " ")
		if #parts == 2 then
...
---@param world string[]
---@return boolean, string?
function libopk.writeWorld(root, world)
	return writefile(root .. "/etc/opk/world", table.concat(world, "\n") .. "\n")
end

---@param root string
---@param installed opk.installedData[]
---@return boolean, string?
function libopk.writeInstalled(root, installed)
	local t = {}
	for _, info in ipairs(installed) do
		table.insert(t, info.package .. " " .. info.version)
	end
	return writefile(root .. "/etc/opk/installed", table.concat(t, "\n") .. "\n")
end
